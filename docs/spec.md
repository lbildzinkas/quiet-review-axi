...
| Thread resolution and its resolver | GraphQL `pullRequest.reviewThreads { isResolved, resolvedBy, comments }` |
| The PR's commits after a comment (label evidence, 10.5) | REST `GET /repos/{owner}/{repo}/pulls/{n}/commits` (paginated) |
| Base-branch commits after the merge, and their files (follow-up fixes, 10.5) | REST `GET /repos/{owner}/{repo}/commits` (paginated), `GET /repos/{owner}/{repo}/commits/{sha}` |
