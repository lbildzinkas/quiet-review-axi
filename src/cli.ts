...
import { ABLATE_HELP, ablateCommand } from './commands/ablate.js'
...
const TOP_LEVEL_HELP = `${encode({
  ...
  commands: {
    ...
    ablate:
      "Compare context variants on a labelled replay: AUROC, noise collapsed and token cost against the replay's baseline",
  ...
  try {
    await runAxiCli<AppContext>({
      ...
      commands: {
        ...
        ablate: (args, ctx) => ablateCommand(args, ctx ?? context),
      ...
      getCommandHelp: (command) => {
        ...
        if (command === 'ablate') return `${ABLATE_HELP}\n`
        return null
      },
      formatError: (error) => {
