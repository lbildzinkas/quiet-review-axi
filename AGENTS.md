...
- Question wording lives only in `src/core/question-pack.json` (spec 5.4.5) and, for the context ablation (`ablate`, spec 10.11), `src/core/question-pack-context.json`; the label-check prompt lives only in `src/replay/label-prompt.json` (spec 10.6). A question wording change is a new pack version gated by `quiet-review-axi gate` against an evaluated replay, not by unit tests; `smoke` is the by-hand check after a Jev model update. A label-prompt change is a new prompt version, which relabels the check sample. The automatic labelling rules live in `src/replay/label.ts` behind `LABEL_RULES_VERSION` (per-bot resolve settings included): a rule change is a new version that must not relabel an existing replay — it ships under a new replay name (`label-rules-v1` froze `public-v1`; the current rules are `label-rules-v2`, revised after the public-v1 adjudication).
...
- Context blocks (spec 10.11) must hold only what existed when the comment was written (edit history and timeline events at or before that time, the file at the comment's commit): later data is what the labels come from and would leak the answer.
