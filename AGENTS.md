...
- Question wording lives only in `src/core/question-pack.json` (spec 5.4.5), and the label-check prompt only in `src/replay/label-prompt.json` (spec 10.6). A wording change is a new version, gated by the replay, not by unit tests.
