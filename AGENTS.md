...
- Question wording lives only in `src/core/question-pack.json` (spec 5.4.5). A wording change is a new pack version gated by `quiet-review-axi gate` against an evaluated replay, not by unit tests; `smoke` is the by-hand check after a Jev model update.
